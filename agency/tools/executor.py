...
class ToolExecutor:
    ...
    def _exec_check_card_balance(
        self, input: Dict, id: str, ctx: Dict
    ) -> ToolResult:
        """
        Check prepaid card balance by delegating to the browser agent.

        Reads card details from credentials.toml, then delegates a browser
        task to visit vanillagift.com and read the balance.
        """
        if not config.DELEGATION_ENABLED:
            return ToolResult(
                tool_use_id=id,
                tool_name="check_card_balance",
                content="Delegation must be enabled for balance checks",
                is_error=True
            )

        from agency.tools.browser.credentials import get_credential

        card_service = getattr(config, 'PREPAID_CARD_SERVICE', 'visa_prepaid')
        cred = get_credential(config.BROWSER_CREDENTIALS_PATH, card_service)
        if cred is None:
            return ToolResult(
                tool_use_id=id,
                tool_name="check_card_balance",
                content=f"No credentials found for '{card_service}'. Add a [{card_service}] section to credentials.toml.",
                is_error=True
            )

        card_number = cred.get("card_number", "")
        cvv = cred.get("cvv", "")
        exp = cred.get("exp", "")
        balance_url = cred.get("balance_check_url", "https://www.vanillagift.com")

        task = (
            f"Check the balance on a prepaid Visa gift card.\n\n"
            f"1. Navigate to {balance_url}\n"
            f"2. Find the balance check section/form\n"
            f"3. Enter the card number: {card_number}\n"
            f"4. Enter the expiration date: {exp}\n"
            f"5. Enter the CVV/security code: {cvv}\n"
            f"6. Submit the form and read the available balance\n"
            f"7. Report the balance amount and any pending charges shown\n\n"
            f"If the page has a CAPTCHA or requires additional verification, "
            f"report that as a blocker."
        )

        try:
            from agency.tools.delegate import run_delegated_task
            result_text = run_delegated_task(task=task, max_rounds=15)
            return ToolResult(
                tool_use_id=id,
                tool_name="check_card_balance",
                content=result_text
            ...
            log_error(f"Balance check delegation failed: {e}")
            return ToolResult(
                tool_use_id=id,
                tool_name="check_card_balance",
                content=f"Balance check failed: {str(e)}",
                is_error=True
            )

    def _exec_initiate_purchase(
        self, input: Dict, id: str, ctx: Dict
    ) -> ToolResult:
        """
        Phase 1: Delegate browses to merchant, fills cart, goes to checkout,
        and reports back details WITHOUT submitting payment.
        """
        if not config.DELEGATION_ENABLED:
            return ToolResult(
                tool_use_id=id,
                tool_name="initiate_purchase",
                content="Delegation must be enabled for purchases",
                is_error=True
            )

        merchant_url = input.get("merchant_url", "")
        instructions = input.get("instructions", "")
        category = input.get("category", "physical")

        if not merchant_url or not instructions:
            return ToolResult(
                tool_use_id=id,
                tool_name="initiate_purchase",
                content="Both merchant_url and instructions are required",
                is_error=True
            )

        from agency.tools.browser.credentials import get_credential
        from agency.spending.manager import SpendingManager

        # Load card credentials for address info
        card_service = getattr(config, 'PREPAID_CARD_SERVICE', 'visa_prepaid')
        cred = get_credential(config.BROWSER_CREDENTIALS_PATH, card_service)
        if cred is None:
            return ToolResult(
        ...
        # Create a pending spending log entry
        card_last_four = cred.get("card_number", "")[-4:] if cred.get("card_number") else ""
        manager = SpendingManager()
        log_id = manager.create_entry(
        ...
        # Build address block for the delegate
        address_info = (
            f"Shipping/Billing address details:\n"
            f"  Name: {cred.get('name_on_card', 'GIFT CARD')}\n"
            f"  Billing address: {cred.get('billing_address', '')}\n"
            f"  Billing city: {cred.get('billing_city', '')}\n"
            f"  Billing state: {cred.get('billing_state', '')}\n"
            f"  Billing zip: {cred.get('billing_zip', '')}\n"
            f"  Shipping address: {cred.get('shipping_address', '')}\n"
            f"  Shipping city: {cred.get('shipping_city', '')}\n"
            f"  Shipping state: {cred.get('shipping_state', '')}\n"
            f"  Shipping zip: {cred.get('shipping_zip', '')}\n"
        )

        task = (
            f"Navigate to a merchant and prepare a purchase — DO NOT submit payment.\n\n"
            f"Merchant URL: {merchant_url}\n\n"
            f"Instructions:\n{instructions}\n\n"
            f"{address_info}\n"
            f"Steps:\n"
            f"1. Navigate to the merchant URL\n"
            f"2. Follow the instructions above to add items to cart\n"
            f"3. Proceed to checkout\n"
            f"4. Enter shipping/billing address details where required\n"
            f"5. STOP before entering payment card details\n"
            f"6. Read the checkout/order summary page carefully\n"
            f"7. Report back the complete order summary: items, subtotal, tax, "
            f"shipping cost, total, and any notes about the checkout page\n\n"
            f"CRITICAL: Do NOT enter payment information. Do NOT submit the order. "
            f"Only report the checkout summary so it can be reviewed before payment."
        )

        try:
            from agency.tools.delegate import run_delegated_task
            result_text = run_delegated_task(task=task, max_rounds=20)

            # Save delegate report and update status
            manager.update_status(
                log_id, "approved",
                delegate_report=result_text,
            )
            ...
            return ToolResult(
                ...
                tool_name="initiate_purchase",
                content=(
                    f"SPENDING LOG #{log_id} — Review checkout details below:\n\n"
                    f"{result_text}\n\n"
                    f"---\n"
                    f"To proceed: call complete_purchase(spending_log_id={log_id})\n"
                    f"To cancel:  call discard_purchase(spending_log_id={log_id})"
                )
            )

        except Exception as e:
            log_error(f"Purchase initiation failed: {e}")
            manager.update_status(log_id, "failed", notes=str(e))
            return ToolResult(
                tool_use_id=id,
                tool_name="initiate_purchase",
                content=f"Purchase initiation failed: {str(e)}",
                is_error=True
            )

    def _exec_complete_purchase(
        self, input: Dict, id: str, ctx: Dict
    ) -> ToolResult:
        """
        Phase 2: Delegate enters payment details and submits the order.
        Browser session cookies persist from Phase 1.
        """
        if not config.DELEGATION_ENABLED:
            return ToolResult(
                tool_use_id=id,
        ...
        spending_log_id = input.get("spending_log_id")
        payment_instructions = input.get("payment_instructions", "")

        if spending_log_id is None:
            return ToolResult(
                tool_use_id=id,
                tool_name="complete_purchase",
                content="spending_log_id is required",
                is_error=True
            )
        ...
        manager = SpendingManager()
        entry = manager.get_entry(spending_log_id)
        if entry is None:
            return ToolResult(
                tool_use_id=id,
                tool_name="complete_purchase",
                content=f"Spending log entry #{spending_log_id} not found",
                is_error=True
            )
        ...
        if entry["status"] != "approved":
            return ToolResult(
                tool_use_id=id,
                tool_name="complete_purchase",
                content=f"Entry #{spending_log_id} has status '{entry['status']}', expected 'approved'",
                is_error=True
            )
        ...
        if cred is None:
            return ToolResult(
                tool_use_id=id,
                tool_name="complete_purchase",
                content=f"No credentials found for '{card_service}'",
                is_error=True
            )

        extra = f"\n\nAdditional instructions: {payment_instructions}" if payment_instructions else ""

        task = (
            f"Complete a purchase by entering payment details and submitting the order.\n\n"
            f"The checkout page should still be available from a previous browser session. "
            f"If the checkout session has expired, navigate back to the merchant and re-add items.\n\n"
            f"Merchant: {entry['merchant']}\n"
            f"Expected items: {entry['description']}\n\n"
            f"Payment card details:\n"
            f"  Card number: {cred.get('card_number', '')}\n"
            f"  Expiration: {cred.get('exp', '')}\n"
            f"  CVV: {cred.get('cvv', '')}\n"
            f"  Name on card: {cred.get('name_on_card', 'GIFT CARD')}\n"
            f"  Billing zip: {cred.get('billing_zip', '')}\n\n"
            f"Steps:\n"
            f"1. If on checkout page, enter the card payment details above\n"
            f"2. Submit/place the order\n"
            f"3. Wait for the confirmation page\n"
            f"4. Report back: order number/confirmation, final total charged, "
            f"and any activation keys, download links, or account details shown"
            f"{extra}"
        )
        ...
        try:
            ...
            manager.update_status(
                spending_log_id, "completed",
                notes=result_text[:500],
            )
            ...
            return ToolResult(
                ...
                tool_name="complete_purchase",
                content=(
                    f"PURCHASE COMPLETED — Spending log #{spending_log_id}\n\n"
                    f"{result_text}\n\n"
                    f"---\n"
                    f"Remember to update this entry with any activation keys, "
                    f"product URLs, or new account credentials if applicable."
                )
            )

        except Exception as e:
            log_error(f"Purchase completion failed: {e}")
            manager.update_status(spending_log_id, "failed", notes=str(e))
            return ToolResult(
                tool_use_id=id,
                tool_name="complete_purchase",
                content=f"Purchase completion failed: {str(e)}",
                is_error=True
            )

    def _exec_discard_purchase(
        self, input: Dict, id: str, ctx: Dict
    ) -> ToolResult:
        """Discard a pending/approved purchase without completing it."""
        from agency.spending.manager import SpendingManager
        ...
        if entry["status"] not in ("pending", "approved"):
            return ToolResult(
                tool_use_id=id,
                tool_name="discard_purchase",
                content=f"Entry #{spending_log_id} has status '{entry['status']}', cannot discard",
                is_error=True
            )

        notes = f"Discarded: {reason}" if reason else "Discarded by user decision"
        manager.update_status(spending_log_id, "discarded", notes=notes)

        return ToolResult(
            tool_use_id=id,
            tool_name="discard_purchase",
            content=f"Purchase #{spending_log_id} discarded. {f'Reason: {reason}' if reason else ''}"
        )

    def _exec_query_spending(
        self, input: Dict, id: str, ctx: Dict
    ) -> ToolResult:
        """Query spending log — financial ledger + digital asset lookup."""
        from agency.spending.manager import SpendingManager

        manager = SpendingManager()
        spending_log_id = input.get("spending_log_id")
        include_keys = input.get("include_keys", True)

        # Single entry lookup
        if spending_log_id is not None:
            entry = manager.get_entry(spending_log_id)
            if entry is None:
                return ToolResult(
                    tool_use_id=id,
                    tool_name="query_spending",
                    content=f"Spending log entry #{spending_log_id} not found",
                    is_error=True
                )
            return ToolResult(
                tool_use_id=id,
                tool_name="query_spending",
                content=self._format_spending_entry(entry, include_keys)
            )

        # Filtered query
        status = input.get("status")
        merchant = input.get("merchant")
        entries = manager.query_log(status=status, merchant=merchant)

        if not entries:
            return ToolResult(
                tool_use_id=id,
                tool_name="query_spending",
                content="No spending log entries found matching the filters."
            )

        # Also get summary
        summary = manager.get_summary()

        parts = [f"SPENDING LOG — {len(entries)} entries"]
        if summary["total_completed_cents"]:
            total_dollars = summary["total_completed_cents"] / 100
            parts.append(f"Total spent (completed): ${total_dollars:.2f}")
        parts.append("")

        for entry in entries:
            parts.append(self._format_spending_entry(entry, include_keys))
            parts.append("---")
    ...
    def _format_spending_entry(self, entry: Dict, include_keys: bool = True) -> str:
        """Format a single spending log entry for display."""
        parts = [
            f"#{entry['id']} | {entry['status'].upper()} | {entry['merchant']}",
            f"  Description: {entry['description']}",
            f"  Category: {entry['category'] or 'physical'}",
            f"  Created: {entry['created_at']}",
        ]

        if entry.get("amount_cents"):
            parts.append(f"  Amount: ${entry['amount_cents'] / 100:.2f}")

        if entry.get("card_last_four"):
            parts.append(f"  Card: ****{entry['card_last_four']}")

        if include_keys:
            if entry.get("activation_key"):
                parts.append(f"  Activation key: {entry['activation_key']}")
            if entry.get("product_url"):
                parts.append(f"  Product URL: {entry['product_url']}")
            if entry.get("license_info"):
                parts.append(f"  License: {entry['license_info']}")

        if entry.get("notes"):
            parts.append(f"  Notes: {entry['notes'][:200]}")

        if entry.get("managed_account_id"):
            parts.append(f"  Linked account ID: {entry['managed_account_id']}")

        return "\n".join(parts)

    def _exec_store_managed_account(
        self, input: Dict, id: str, ctx: Dict
    ) -> ToolResult:
        """Store credentials for a dynamically-created account."""
        from agency.spending.manager import AccountManager, SpendingManager
        ...
        if not service or not login or not password:
            return ToolResult(
                tool_use_id=id,
                tool_name="store_managed_account",
                content="service, login, and password are all required",
                is_error=True
            )

        account_mgr = AccountManager()

        # Check for existing account with this service name
        existing = account_mgr.get_account(service)
        if existing:
            return ToolResult(
                tool_use_id=id,
                tool_name="store_managed_account",
                content=(
                    f"An account for '{service}' already exists (ID #{existing['id']}). "
                    f"Use a different service name or update the existing account."
                ),
                is_error=True
            )

        try:
            account_id = account_mgr.store_account(
                service=service,
                login=login,
                password=password,
                pin=input.get("pin"),
                email_used=input.get("email_used"),
                login_url=input.get("login_url"),
                spending_log_id=input.get("spending_log_id"),
            )

            # If linked to a spending log entry, update the cross-reference
            spending_log_id = input.get("spending_log_id")
            if spending_log_id:
                spending_mgr = SpendingManager()
                entry = spending_mgr.get_entry(spending_log_id)
                if entry:
                    spending_mgr.update_status(
                        spending_log_id,
                        entry["status"],  # Preserve current status
            ...
            return ToolResult(
                ...
                tool_name="store_managed_account",
                content=(
                    f"Account stored: '{service}' (ID #{account_id})\n"
                    f"The delegate can now use get_credentials('{service}') to access this account."
                )
            )

        except Exception as e:
            log_error(f"Failed to store managed account: {e}")
            return ToolResult(
                tool_use_id=id,
                tool_name="store_managed_account",
                content=f"Failed to store account: {str(e)}",
                is_error=True
            )
