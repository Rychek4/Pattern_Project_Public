...
# PyQt5 is optional — only required for the desktop GUI (--gui mode).
# The web server and engine modules import ProcessEventType, ProcessEvent, and
# get_process_event_bus() which work without Qt.  The panel widgets below are
# only instantiated when the GUI is running.
try:
...
def _make_process_event_bus():
    """Create the ProcessEventBus class with or without Qt support.

    When PyQt5 is available the class inherits QObject and emits a
    pyqtSignal so the desktop GUI's ProcessPanel can connect directly.
    Without PyQt5 (web/CLI modes) the class is a plain Python object
    that only uses callback-based dispatch.
    """

    if _HAS_QT:
        class _ProcessEventBus(QObject):
            """Central event bus for process panel updates (Qt-enabled)."""
            ...
            def add_callback(self, callback) -> None:
                """Register a plain callback for non-Qt subscribers."""
        ...
        class _ProcessEventBus:
            """Central event bus for process panel updates (no-Qt fallback)."""
    ...
    return _ProcessEventBus


ProcessEventBus = _make_process_event_bus()
...
# =============================================================================
# QT WIDGETS (only available when PyQt5 is installed)
# =============================================================================

if not _HAS_QT:
    # Provide stubs so that "from interface.process_panel import ProcessPanel"
    # gives a clear error message instead of a confusing NameError.
    class ProcessNodeWidget:  # type: ignore[no-redef]
        def __init__(self, *a, **kw):
            raise RuntimeError("ProcessNodeWidget requires PyQt5 (--gui mode)")

    class RoundGroupWidget:  # type: ignore[no-redef]
        def __init__(self, *a, **kw):
            raise RuntimeError("RoundGroupWidget requires PyQt5 (--gui mode)")

    class MessageSeparatorWidget:  # type: ignore[no-redef]
        def __init__(self, *a, **kw):
            raise RuntimeError("MessageSeparatorWidget requires PyQt5 (--gui mode)")

    class MessageGroupWidget:  # type: ignore[no-redef]
        def __init__(self, *a, **kw):
            raise RuntimeError("MessageGroupWidget requires PyQt5 (--gui mode)")

    class ProcessPanel:  # type: ignore[no-redef]
        def __init__(self, *a, **kw):
            raise RuntimeError("ProcessPanel requires PyQt5 (--gui mode)")

else:  # _HAS_QT is True -- define the real widgets
