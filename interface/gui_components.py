...
class SpellCheckHighlighter(QSyntaxHighlighter):
    ...
    Uses pyspellchecker for spell checking and QSyntaxHighlighter
    to apply red underlines to misspelled words in real-time.
    """
    ...
    def __init__(self, document, theme: Theme = None):
        ...
        # Lazy load spell checker to avoid import errors if not installed
        self._spell = None
        self._load_spell_checker()

        # Setup text format for misspelled words
        self._error_format = QTextCharFormat()
        self._update_error_format()
    ...
    def _load_spell_checker(self):
        ...
        try:
            ...
            # pyspellchecker not installed - spell check will be disabled
            self._spell = None

    def _update_error_format(self):
        """Update the error format based on current theme."""
        self._error_format.setUnderlineStyle(QTextCharFormat.SpellCheckUnderline)
        # Use theme error color or fallback to red
        error_color = self._theme.error if self._theme else "#ef4444"
    ...
    def highlightBlock(self, text: str):
        """
        Called by Qt for each block (line) of text.
        Identifies misspelled words and applies underline formatting.
        """
        if not self._enabled or self._spell is None:
            return

        # Find all words in the text
        # Match word characters, but handle contractions (e.g., don't, it's)
        word_pattern = re.compile(r"\b[a-zA-Z']+\b")

        for match in word_pattern.finditer(text):
            word = match.group()

            # Skip very short words and words that are all uppercase (likely acronyms)
            if len(word) <= 1 or word.isupper():
                continue

            # Strip leading/trailing apostrophes for checking
            check_word = word.strip("'")
            if not check_word:
                continue

            # Check if word is misspelled
            if check_word.lower() not in self._spell:
                # Apply error formatting
                self.setFormat(match.start(), match.end() - match.start(), self._error_format)
    ...
    def is_misspelled(self, word: str) -> bool:
        ...
        if not check_word or len(check_word) <= 1:
            return False

        return check_word not in self._spell

    def add_to_dictionary(self, word: str):
        """Add a word to the spell checker's known words."""
