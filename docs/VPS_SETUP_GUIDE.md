...
**What setup.sh does:**
1. Installs system packages (Python 3, nginx, certbot, git, curl)
...
5. Creates a template `.env` file (with `chmod 600`)
...
# Email gateway — uncomment if using
# EMAIL_GATEWAY_ENABLED=true
# APP_EMAIL_ADDRESS=you@gmail.com
# APP_EMAIL_PASS=your-gmail-app-password
...
launch_command = "python main.py --web"
...
> **Note:** The `launch_command` uses `--web` mode (not `--cli` as in the spec
> examples) because the VPS deployment uses the FastAPI web interface.
...
| `CLARIFICATION_ENABLED` | true | **Keep on** | AI can ask you questions |
...
| `EMAIL_GATEWAY_ENABLED` | false | Your choice | Requires Gmail app password |
| `REDDIT_ENABLED` | false | Your choice | Requires Reddit API credentials |
| `MOLTBOOK_ENABLED` | false | Your choice | Requires Moltbook API key |
| `CLIPBOARD_ENABLED` | true | **Disable** | No X11/desktop on VPS |
| `VISUAL_ENABLED` | true | **Disable** | No display/webcam on VPS |

## Appendix B: The .env Template Generated by setup.sh

For reference, `deploy/setup.sh` creates a minimal `.env` with just the essentials.
The full `.env.example` in the repo root documents every available variable. You can
use it as a reference:

```bash
# View all available environment variables and their documentation
cat /opt/pattern/.env.example
```
...
| `ReadWritePaths=/opt/pattern/data /opt/pattern/logs` | Only these directories are writable |
| `PrivateTmp=yes` | Gives Pattern its own `/tmp` |
| `LimitNOFILE=65535` | Generous file descriptor limit |

Pattern can only write to its `data/` and `logs/` directories. Everything else
on the filesystem is read-only or invisible to the process.
